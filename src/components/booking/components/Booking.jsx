...
const Booking = (props) => {
  const { deal, accommodation, deals, rooms, children, adults } = useSelector(
    (state) => state.booking,
    shallowEqual
  );

  const { startDate, endDate, title } = ensureNonNull(accommodation);
  ...
  const BookingContainer = () => <Row>
      ...
      <Col sm={3}>
        <div className={"bg-light p-5 text-center rounded shadow mt-4"}>
          ...
          <h4 className={"py-3 mb-0"}>{startDate.diff(endDate)} nights</h4>
  ...
  const Rooms = () => {
    const roomDeals = ensureNonEmpty(deals).map((deal) => (
      <Row
        ...
        key={deal.title}
        onClick={() => console.log(`Select room`, deal)}
      >
    ...
    return <section id={"deals"}>{roomDeals}</section>;
  };
